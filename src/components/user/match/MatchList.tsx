...
const UserCard: React.FC<UserCardProps> = ({ user }) => {
    ...
    return (
        <Card className="w-64 hover:shadow-md transition-shadow">
            ...
            <CardFooter className="flex items-center justify-between pt-2">
                <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full ${matchColor}`}>
...
const MatchList: React.FC = () => {
    ...
    return (
        <div className="bg-background p-2 md:p-6 lg:p-20 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 justify-items-center md:justify-items-start">
            {users.length > 0 ? (
                users.map((user) => <UserCard key={user.id} user={user} />)
            ) : (
                Array.from({ length: 3 }).map((_, index) => <UserSkeleton key={index} />)
            )}
        </div>
    );
};
